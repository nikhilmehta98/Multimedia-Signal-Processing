...
def start_debugging(testcase=0, nsteps_to_run=len(submitted.steps)):
    '''(dataset, step, solution, error) = debug.start_debugging(testcase, nsteps_to_run)

    This function reloads your submitted.py file (in case you have changed it).
    It then creates a new Dataset object, using the specified testcase (default:0),
    ...
    If a solution.npz file is available for your specified testcase, it is loaded as 
    ...
    solutions_filename = os.path.join('solutions','testcase%d_solution.npz'%(testcase))
    step = submitted.steps[nsteps_to_run-1]
    if not os.path.isfile(solutions_filename):
        error = 'No reference solution available for testcase %d'%(testcase)
        solution = {}
    else:
